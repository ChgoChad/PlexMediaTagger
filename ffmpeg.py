...
class FFMpeg(object):
    ...
    def _spawn(cmds, communicate=False):
        #Log.Debug('Command line to be sent to ffmpeg: %s' % cmds)
        if Popen and os.name != 'nt':
            ...
            p = Popen(cmds, shell=False, stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=False)
            if communicate:
                return p.communicate()
            else:
                return (p.stdout, p.stderr)
    ...
    def probe(self, fname):
        ...
        raw, _ = self._spawn([self.ffprobe_path, '-show_format', '-show_streams', fname], True)
    ...
    def convert(self, infile, outfile, opts, timeout=10):
        ...
        #Log,Debug('Cmds sending to ffmpeg: %s' % cmds)
